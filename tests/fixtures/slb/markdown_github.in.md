...
1. Install the tool.
2. Run it in check mode first, so that you can see what would change before anything is written to disk, and
   only then run the fix mode.
   1. Nested ordered items keep their own indentation.

> A blockquote that was hard wrapped by hand
> in the middle of a clause is joined and broken again at the boundary that reads best.
