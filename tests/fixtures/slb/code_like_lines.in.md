...
## Code lines inside prose paragraphs

A single code line keeps the whole paragraph it sits in as written, even without a semicolon at its end.

Print the resolved value with
println!("{}", config.width)
and compare it with the file.

Inspect the parsed paragraph with
dbg!(&paragraph.lines)
before the reflow runs.

Every generated file starts with
// Code generated by protoc. DO NOT EDIT.
so nobody changes it by hand.

The script must start with
#!/usr/bin/env bash
or the runner falls back to sh.

Paste this into the REPL to get started
