...
# The C++ standard is pinned below so every host builds the same dialect.
# Extensions stay off because they hide portability problems until a second compiler is tried.
...
if(APPLE)
    # Use libc++ on macOS when using brew LLVM.
    # Otherwise the link can fail if brew clang is mixed with the system libc++.
...
if(WIN32)
    # Set the OpenSSL search path for Windows.
    # Scoop, Chocolatey, and the slproweb installer each use a different layout.
    ...
    elseif(EXISTS "C:/Program Files/OpenSSL/include")
        ...
        # The slproweb Win64OpenSSL installer (used by Chocolatey) places libraries in subdirectories
        # that CMake's FindOpenSSL doesn't search by default.
...
# OpenSSL is required at link time.
# The Crypto target is enough because the game only hashes the board, it never opens a TLS socket.
...
if(MSVC)
    ...
    # Skip native CPU targeting on CI:
    # cached build artifacts are shared between runners with different CPUs,
    # so natively-targeted code can crash with SIGILL on another runner.
