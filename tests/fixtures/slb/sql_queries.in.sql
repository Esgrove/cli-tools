...
-- A comment block that is already
-- formatted correctly stays as it is.
CREATE INDEX idx_transactions_account ON transactions (account_id);

-- Ranks every account by its total spend within the month and keeps only the top ten; ties
-- are broken by account identifier so the result is stable across runs on the same data.
