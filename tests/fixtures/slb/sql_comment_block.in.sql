...
-- Backfill the new column with a default derived from the existing status column; this runs once
-- and is safe to re-run because the WHERE clause only touches rows that still have the old default.
